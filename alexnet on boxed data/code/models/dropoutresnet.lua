...
function createModel(opt)
  ...
  layer(model, opt.deathRate, 64, 64, def[1], 1) -- 32
  layer(model, opt.deathRate, 64, 128, def[2], 2) -- 16
  layer(model, opt.deathRate, 128, 256, def[3], 2) -- 8
  layer(model, opt.deathRate, 256, 512, def[4], 2) -- 4
  model:add(Avg(7,7,1,1)):add(nn.Reshape(512))
  model:add(nn.Linear(nFeatures, 3))
